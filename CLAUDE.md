...
| `edit <task_id> [--title ...] [--due ... \| --clear-due] [--start ... \| --clear-start] [--priority ...] [--dry-run]` | Edit mutable task fields (title, content, due/start dates, priority). `--dry-run` previews the PATCH body without calling the API. | `POST /open/v1/task/{taskId}` |
| `punt <task_id> <when> [--dry-run]` | Sugar over `edit --start` — push start date forward | `POST /open/v1/task/{taskId}` |
| `bump <task_id> {none,low,medium,high} [--dry-run]` | Sugar over `edit --priority` — set priority by name | `POST /open/v1/task/{taskId}` |
