...
| `edit <task_id> [--title ...] [--due ... \| --clear-due] [--start ... \| --clear-start] [--priority ...] [--dry-run] [--full]` | Edit mutable task fields (title, content, due/start dates, priority). `--dry-run` previews the PATCH body without calling the API. `--full` prints the entire TickTick task response instead of the abridged summary. | `POST /open/v1/task/{taskId}` |
| `punt <task_id> <when> [--dry-run] [--full]` | Sugar over `edit --start` — push start date forward | `POST /open/v1/task/{taskId}` |
| `bump <task_id> <level> [--dry-run] [--full]` | Sugar over `edit --priority` — `<level>` is a name (none/low/medium/high) or canonical int (0/1/3/5) | `POST /open/v1/task/{taskId}` |
