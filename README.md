...
| `bump <task_id> <level> [--dry-run] [--full]` | Sugar over `edit --priority`. `<level>` is a name (`none`/`low`/`medium`/`high`) or canonical numeric priority (`0`/`1`/`3`/`5`). `--dry-run` previews the PATCH body; `--full` prints the entire task response. |
